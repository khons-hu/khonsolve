...
addEventListener('message',event=>{
 ...
 try{
  url=URL.createObjectURL(new Blob(['('+(language==='python'?pythonWorkerMain:workerMain).toString()+')();'],{type:'text/javascript'}));worker=new Worker(url);
  timer=setTimeout(()=>{cleanup();reply({type:'error',message:'Runtime initialization timed out. Try again.'});},language==='python'?30000:language==='typescript'?10000:2000);
  worker.onmessage=e=>{const data=e.data;if(data?.type==='executing'){clearTimeout(timer);timer=setTimeout(()=>{cleanup();reply({type:'error',message:'Stopped after 2 seconds. Check for an infinite loop or a slow algorithm.'});},2000);reply(data);return;}cleanup();if(data?.type==='result'&&Array.isArray(data.results))reply(data);else reply({type:'error',message:String(data?.message||'Runner error')});};
